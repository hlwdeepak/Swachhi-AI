...
export async function GET(req: NextRequest) {
  ...
  const pending = (db.prepare("SELECT COUNT(*) as c FROM complaints WHERE status != 'resolved'").get() as any).c;
  ...
  const ward_stats = wardRows.map((w: any) => {
    ...
    return {
      ...
      collection_efficiency: w.collection_efficiency,
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    trend.push({ date: label, complaints: d, resolved: r });
  }

  const hotspots = (db.prepare("SELECT latitude as lat, longitude as lng, ward, category, priority FROM complaints WHERE status != 'resolved' AND latitude IS NOT NULL").all() as any[]);
